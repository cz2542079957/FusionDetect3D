...
YOLO::YOLO(vector<string> &classes,  vector<uchar> &bufferCfg, vector<uchar> &bufferModel)
{
    this->confThreshold = 0.5;
    this->nmsThreshold = 0.4;
    this->inpWidth = 320;
    this->inpHeight = 320;
    strcpy(this->netname, "yolo-fastest");
    this->classes.assign(classes.begin(), classes.end());
    this->net = readNetFromDarknet(bufferCfg, bufferModel);
...
void YOLO::detect(Mat &frame)
{
    ...
    string label = format("%s: %.2f ms", this->netname, t);
